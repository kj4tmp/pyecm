...
if sys.version_info < (3, 11):
    try:
        ...
        raise RuntimeError("stubgen.py requires the 'typing_extensions' package on Python <3.11")
else:
...
class StubGen:
    def __init__(
        ...
        self.id_seq = re.compile(
            sep_before + "((?:" + identifier + r"\.)+)(" + identifier + r")\b" + sep_after
        )
    ...
    def put_nb_overload(
        self, fn: NbFunction, sig: NbFunctionSignature, name: Optional[str] = None
    ) -> None:
        """
        ...
        if default_args:
            for index, arg in enumerate(default_args):
                ...
                assert "\n" not in arg_str, "Default argument string may not contain newlines."
    ...
    def put_function(
        self, fn: Callable[..., Any], name: Optional[str] = None, parent: Optional[object] = None
    ):
    ...
    def put_type(self, tp: NbType, name: Optional[str]):
        ...
        if name and (name != tp_name or mod_name != tp_mod_name):
            ...
            elif self.include_external_imports or (
                same_toplevel_module and self.include_internal_imports
            ):
    ...
    def put_value(
        self, value: object, name: str, parent: Optional[object] = None, abbrev: bool = True
    ) -> None:
        """
        ...
        if isinstance(parent, type) and issubclass(tp, parent):
            ...
            same_toplevel_module = (
                named_value.__module__.split(".")[0] == self.module.__name__.split(".")[0]
            )

            if self.include_external_imports or (
                same_toplevel_module and self.include_internal_imports
            ):
    ...
    def put(
        self, value: object, name: Optional[str] = None, parent: Optional[object] = None
    ) -> None:
    ...
    def import_object(self, module: str, name: Optional[str], as_name: Optional[str] = None) -> str:
        """
    ...
    def expr_str(self, e: Any, abbrev: bool = True) -> Optional[str]:
        ...
        elif (sys.version_info >= (3, 11) and issubclass(tp, typing.TypeVarTuple)) or (
            typing_extensions is not None and issubclass(tp, typing_extensions.TypeVarTuple)
        ):
    ...
    def type_str(self, tp: Union[List[Any], Tuple[Any, ...], Dict[Any, Any], Any]) -> str:
        ...
        if isinstance(tp, typing.TypeVar):
            ...
            dict_gen: Generator[str, None, None] = (
                repr(k) + ": " + self.type_str(v) for k, v in tp.items()
            )
            return "{" + ", ".join(dict_gen) + "}"
        elif origin and args:
            args_gen: Generator[str, None, None] = (self.type_str(a) for a in args)
            result = self.type_str(origin) + "[" + ", ".join(args_gen) + "]"
...
def parse_options(args: List[str]) -> argparse.Namespace:
    ...
    if len(opt.modules) > 1 and opt.output_file:
        parser.error("The -o option can only be specified when a single module is being processed.")
    if opt.recursive and opt.output_file:
        parser.error("The -o option is not compatible with recursive stub generation (-r).")
    return opt
