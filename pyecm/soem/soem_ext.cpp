...
NB_MODULE(soem_ext, m)
{
    ...
    nb::class_<SOEM_wrapper>(m, "SOEM")
        ...
        .def("init_redundant", &SOEM_wrapper::init_redundant, "ifname"_a, "if2name"_a, "Initialize SOEM library with cable redundancy. Returns > 0 if OK.")
        ...
        .def("init", &SOEM_wrapper::init, "ifname"_a, "Initialize SOEM library. Returns > 0 if OK.")
        .def("config_init", &SOEM_wrapper::config_init, "Enumerate and request INIT state for all subdevices. Returns workcounter of subdevice discovery datagram which is the number of subdevices found.")
